class ZoomGSeriesMessageEncoder implements MessageEncoder {

	//@Override
	encode(messages : Messages) : MidiMessages {
	...
	toPatchMessages(message) : MidiMessages {
	...
	statusEffectMessages(message : Message, cause : CommonCause) : MidiMessages{
	...
	setParamMessages(message : Message) : MidiMessages {
		let effect = message.details.effect;
	...
	setEffectMessages(message : Message) : MidiMessages {
		let effect = message.details.effect;
		let value  = message.details.value;
	...
	private static SET_STATUS = 0;
	private static CHANGE_EFFECT = 1;
	private static PARAM_EFFECT = 2; // Base

	manipuleEffectMessages(effect : number, type : number, value : number) : MidiMessages {
	...
	// SPECIFIC ZOOM
	///////////////////////////////////////
	requestCurrentPatchNumberMessages(message : Message) : MidiMessages {
	...
	requestCurrentPatchDetailsMessages(message : Message) : MidiMessages {
	...
	requestSpecificPatchDetailsMessages(message) : MidiMessages {
		let patch = message.details.patch;
	...
	youCanTalkMessages() : MidiMessages {
